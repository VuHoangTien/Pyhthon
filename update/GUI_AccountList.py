...
#         headers = ["Tên đăng nhập", "Mật khẩu", "Gmail", "Số điện thoại", "Thao tác"]
#         header_widths = [180, 160, 250, 150, 200]  # 5 cột tương ứng
...
#                                    command=lambda p=account: self.view_account_detail(p))
...
#     def open_account_detail_window(self, patient_data):
...
#         detail_gui = AccountDetailGui(self.master, patient_data)
        
#     def view_account_detail(self, patient_data):
#         """Xem chi tiết bệnh nhân"""
#         if self.navigation_callback:
#             # Sử dụng callback để chuyển màn hình (dành cho tích hợp với GUI_Main)
#             self.navigation_callback('staffs_detail', patient_data)
#         else:
#             # Mở trực tiếp (dành cho test độc lập)
#             self.open_account_detail_window(patient_data)
